...
/** Represents an async thunk action creator to add an element to the maintenance entries. */
export const addMaintenenceEntryAsyncActionCreator = (entry: MaintenanceEntry) => (dispatch: Dispatch) => {
  MaintenenceService.writeMaintenenceEntry(entry).then(result => {
    result && window.setTimeout(() => {
      ...
      dispatch(new AddSnackbarNotification({ message: `Successfully created maintenance settings for [${entry.nodeId}]`, options: { variant: 'success' } }));
    }, 900);
    dispatch(maintenanceEntriesReloadAction);
  });
};

/** Represents an async thunk action creator to delete an element from the maintenance entries. */
...
/** Represents an async thunk action creator to update maintenance entry. */
export const updateMaintenenceEntryAsyncActionCreator = (entry: MaintenanceEntry) => (dispatch: Dispatch) => {
  MaintenenceService.updateMaintenenceEntry(entry).then(result => {
    result && window.setTimeout(() => {
      // dispatch(loadAllMountedNetworkElementsAsync);
      dispatch(new UpdateMaintenanceEntry(entry));
